...
/**
  ...
  * the License.
  */
object Tutorial_08_FlatMap extends App {
  ...
  val donuts2 = Seq("Vanilla", "Glazed")
  ...
  println("\nStep 4: How to append the Donut literal using the map function")
  val listDonutsFromMap: List[Seq[String]] = listDonuts.map(donutSeq => donutSeq.map(_ + " Donut"))
  println(s"Elements of listDonutsFromMap = $listDonutsFromMap")



  println("\nStep 5: How to return a single list of donut using the flatMap function")
  val listDonutsFromFlatMap: List[String] = listDonuts.flatMap(donutSeq => donutSeq.map(_ + " Donut"))
