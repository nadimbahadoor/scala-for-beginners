...
/**
  ...
  * the License.
  */
object Tutorial_08_ForComprehensionsAndLoops extends App {
  ...
  println("\nStep 6: How to use a while loop in Scala")
  ...
  val donutsToBake = List("plain", "vanilla", "chocolate", "strawberry", "glazed")
  val numberOfDonutsBaked = donutsToBake.foldLeft(0){ (count, donut) =>
  ...
  def calcDonutsToBake(list: List[Any]): Int = list.foldLeft(0)((count, _) => count + 1)
  val numberOfDonutsBaked3 = calcDonutsToBake(donutsToBake)
  println(s"number of donuts to bake = $numberOfDonutsBaked3")
}
