...
/**
  ...
  * the License.
  */
object Tutorial_11_Option extends App {
  ...
  def orderDonut(donutName: Option[String]): Unit = ???


  def orderDonut2(donutName: String): Unit = ???



  println("\nTip: Filter None using map function")
  glazedDonutTaste.map(taste => println(s"$taste"))
